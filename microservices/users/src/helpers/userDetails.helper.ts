import {UserDetails as GrpcUserDetails, Response, UpdateUserDetailsRequest, UserDetailsResponse, Id} from '../grpc/_proto/user/user_pb';
...
class UserDetailsHelper {
    ...
    public async UpdateUserDetails(userDetailsData : UpdateUserDetailsRequest) : Promise<UserDetailsResponse> {
        ...
        if (!userDetails) {
            result.setSuccess(false);
            result.setMessage(`User details with id ${userDetailsData.getId()} doesn't exist.`)
    ...
    public async FindUserDetailsByUserId(idData : Id) : Promise<UserDetailsResponse> {
        let result = new UserDetailsResponse();
    ...
    private UpdateUserDetailsProperties (userDetails : IUserDetails, userDetailsDeta: UpdateUserDetailsRequest) : IUserDetails {
        if (isString(userDetailsDeta.getData()?.getFirstname())) {
            userDetails.firstName = userDetailsDeta.getData()?.getFirstname() as string;
        }
    ...
    private async UserDetailsFromDbUserDetails(userDetails : IUserDetails) : Promise<GrpcUserDetails> {
